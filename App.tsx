...
import { Loader2, Database, RefreshCw, AlertTriangle, PlayCircle, Copy, Check, Menu, Activity, Info } from 'lucide-react';
...
-- 3. CRITICAL: Refresh the PostgREST cache so the API recognizes new columns
NOTIFY pgrst, 'reload schema';

-- 4. Enable Row Level Security (RLS)
alter table public.patients enable row level security;

-- 5. Create a broad access policy (for demo/development)
drop policy if exists "Enable access for all users" on public.patients;
           ...
           <div className="max-w-2xl w-full bg-white rounded-2xl shadow-xl overflow-hidden">
             <div className="p-8 border-b border-slate-100">
               <div className="flex items-center gap-4 mb-4">
                 ...
                 <div>
                   <h2 className="text-xl font-bold text-slate-900">Database Cache Sync Required</h2>
                   <p className="text-slate-500 text-sm">Supabase's API cache needs to be refreshed to recognize the 'image_url' and 'clinical_notes' columns.</p>
                 </div>
               </div>
               
               <div className="bg-slate-50 rounded-lg p-4 mb-6 border border-slate-200">
                 <div className="flex items-center gap-2 mb-3 text-amber-700 bg-amber-50 p-3 rounded-lg border border-amber-100">
                   <Info className="w-5 h-5 flex-shrink-0" />
                   <p className="text-sm font-medium">To fix this, run the SQL below and ensure the <strong>NOTIFY pgrst</strong> command is included.</p>
                 </div>
                 ...
                 <p className="text-xs text-slate-500 mt-2">After running this, wait a few seconds and click <strong>Retry Connection</strong>.</p>
               </div>
        ...
        {patients.length === 0 && !loading ? (
           <div className="flex flex-col items-center justify-center flex-1 space-y-6 p-4">
             ...
             <button 
               ...
               disabled={seeding}
               className="flex items-center gap-2 px-6 py-3 bg-teal-600 hover:bg-teal-700 text-white rounded-lg font-semibold transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-teal-600/20"
             >
